...
export const getBeersService = async (filters, currentPage) => {
  const { id, beerName, yeast, date, abv } = filters;
  ...
  const url = `/beers?page=${currentPage}&per_page=${filters.noOfPost}${dateQueryString}${abvQueryString}${beerQueryString}${idQueryString}${yeastQueryString}`;
